...
#========= Task creation ============
	...
	async def task_send(self):
		...
		while(True):
			...
			if regs:
				content = {"type":self._deviceType, "data": regs, "timeStamp": str(datetime.datetime.now(tz=timezone.utc).replace(tzinfo=None))}
	...
	def task_decode(self, valueList, task, taskID):
		...
		for taskDict in dicTaskList:
			...
			if (taskDict["taskID"] == taskID):
				...
				fractionDigit = taskDict["fractionDigit"]
				PF = taskDict["PF"]
				size = taskDict["size"]
				#decode value
				...
				if PF is not None:
					finalValue = round(decodedValue * (10**PF), fractionDigit)
				else:
